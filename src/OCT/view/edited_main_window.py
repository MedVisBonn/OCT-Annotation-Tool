...
class DrawCostPointCommand(QtGui.QUndoCommand):
    ...
    def redo(self):
        ...
        self.info = self.parentWind.draw_cost_point_command_undo_redo(self.x, \
                                                                      self.y, self.smoothness, self.layerName,
                                                                      self.viewName, self.info, \
                                                                      self.sliceNum, 'redo')
...
class DrawSplineCommand(QtGui.QUndoCommand):
    ...
    def redo(self):
        ...
        self.info = self.parentWind.draw_spline_command_undo_redo(self.prevLayer, \
                                                                  self.prevKnots, self.redoLayer, self.redoKnots,
                                                                  self.layerName, \
...
class DrawPolyFitCommand(QtGui.QUndoCommand):
    ...
    def undo(self):
        self.parentWind.draw_poly_fit_command_undo_redo(self.image, self.topLeftX, \
                                                        self.topLeftY, self.bottomRightX, self.bottomRightY,
                                                        self.polyDegree, \
    ...
    def redo(self):
        ...
        self.info = self.parentWind.draw_poly_fit_command_undo_redo(self.image, \
                                                                    self.topLeftX, self.topLeftY, self.bottomRightX,
                                                                    self.bottomRightY, \
                                                                    self.polyDegree, self.layerName, self.viewName,
                                                                    self.info, self.sliceNum, 'redo')
...
class DrawDrusenOnEnfaceCommand(QtGui.QUndoCommand):
    ...
    def undo(self):
        self.parentWind.draw_drusen_on_enface_command_undo_redo(self.x, self.y, \
                                                                self.color, self.viewName, self.info, self.thickness,
                                                                'undo')

    def redo(self):
        self.info = self.parentWind.draw_drusen_on_enface_command_undo_redo(self.x, self.y, \
                                                                            self.color, self.viewName, self.info,
                                                                            self.thickness, 'redo')
...
class AcceptSuggestedSegmentationCommand(QtGui.QUndoCommand):
    ...
    def undo(self):
        self.parentWind.accept_suggest_seg_command_undo_redo(self.layerName, \
                                                             self.info, self.sliceNumZ, self.smoothness, self.uncType,
                                                             self.extent, \
                                                             self.csps, 'undo')
...
class DrawPenCommand(QtGui.QUndoCommand):
    ...
    def undo(self):
        ...
        if (len(self.redoValues) > 0):
            self.parentWind.draw_pen_command_undo_redo(self.x, self.y, c, \
                                                       self.viewName, self.prevValues, self.slices, self.posY,
                                                       self.sliceNum, \
                                                       'undo', self.info, self.layerName)
            return
        self.parentWind.draw_pen_command_undo_redo(self.x, self.y, c, self.viewName, \
                                                   self.prevValues, self.slices, self.posY, self.sliceNum, 'undo',
                                                   self.info, \
                                                   self.layerName)
    ...
    def redo(self):
        ...
        self.info = self.parentWind.draw_pen_command_undo_redo(self.x, self.y, \
                                                               self.color, self.viewName, self.redoValues, self.slices,
                                                               self.posY, \
...
class DrawLineCommand(QtGui.QUndoCommand):
    ...
    def undo(self):
        ...
        self.parentWind.draw_line_command_undo_redo(self.x1, self.y1, self.x2, \
                                                    self.y2, c, self.viewName, self.posX, self.slices, self.posY,
                                                    self.sliceNum, \
    ...
    def redo(self):
        ...
        self.info = self.parentWind.draw_line_command_undo_redo(self.x1, self.y1, \
                                                                self.x2, self.y2, self.color, self.viewName, self.posX,
                                                                self.slices, \
                                                                self.posY, self.sliceNum, self.redoValues, 'redo',
                                                                self.info, self.layerName)
...
class DrawCurveCommand(QtGui.QUndoCommand):
    ...
    def undo(self):
        ...
        self.parentWind.draw_curve_command_undo_redo(self.posX[::-1], \
                                                     self.slices[::-1], self.posY[::-1], c, self.viewName,
                                                     self.sliceNum, \
    ...
    def redo(self):
        ...
        self.info = self.parentWind.draw_curve_command_undo_redo(self.posX, \
                                                                 self.slices, self.posY, self.color, self.viewName,
                                                                 self.sliceNum, \
...
class DrawExtractCommand(QtGui.QUndoCommand):
    ...
    def undo(self):
        ...
        self.parentWind.draw_extract_command_undo_redo(self.xs, self.ys, \
                                                       self.zs, self.xns, self.yns, self.zns, c, self.viewName,
                                                       self.sliceNum)

    def redo(self):
        self.parentWind.oct_controller.set_slice_number(self.sliceNum, self.viewName)
        self.parentWind.draw_extract_command_undo_redo(self.xs, self.ys, self.zs, \
                                                       self.xns, self.yns, self.zns, self.color, self.viewName,
                                                       self.sliceNum)
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.action_Import.triggered.connect(self.import_action)
    ...
    def get_filepath(self, lastPath):
        """
        ...
        # Set window size.
        w.resize(320, 240)
        filepath = QtGui.QFileDialog.getOpenFileName(w, "Import Heidelberg Engineering OCT raw files (.vol ending)",
                                                     lastPath, "HE OCT raw files (*.vol)")
    ...
    def get_save_path(self, lastPath):
        ...
        w.resize(320, 240)
        # Get file name
        filename = QtGui.QFileDialog.getExistingDirectory(w, 'Save Dir', lastPath, QtGui.QFileDialog.ShowDirsOnly)
        return filename
    ...
    def import_action(self):
        ...
        if openFlag:
            status = self.oct_controller.import_vol()
            if (status == 0):
                self.oct_controller.write_in_log(self.oct_controller.get_time() + \
                                                 ',' + self.action_Open.objectName() + ',' + \
                                                 self.oct_controller.get_scan_path() + '\n')
