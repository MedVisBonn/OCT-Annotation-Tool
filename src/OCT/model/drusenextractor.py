...
logging.basicConfig(filename=os.path.join(os.path.expanduser('~'), 'octannotation.log'),
                    ...
                    level=logging.DEBUG,
                    filemode='w')
logger = logging.getLogger('drusenextractor')
...
class DrusenSeg:
    ...
    def get_drusen_seg_polyfit(self, layers):
        ...
        """
        logger.debug('Get drusen segmentation polynomial fit')
        ...
        for i in range(layers.shape[2]):
            ...
            try:
                ...
                logger.debug("Error in get_drusen_seg_polyfit function")
        return self.drusen
    ...
    def get_RPE_layer(self, img):
        ...
        """
        logger.debug('Extract pixel-wise RPE location')
        y = []
        x = []
    ...
    def normal_RPE_estimation(self, layer, degree=3, it=3, s_ratio=1, farDiff=5, ignoreFarPoints=True, returnImg=False,
        ...
        """
        logger.debug('Normal RPE etimation')
        if (useWarping):
    ...
    def warp_BM(self, seg_img, returnWarpedImg=False):
        ...
        yn, xn = self.normal_RPE_estimation(rmask, it=5, useWarping=False, xloc=xr, yloc=shifted_yr)
