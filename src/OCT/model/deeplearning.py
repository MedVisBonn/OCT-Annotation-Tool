...
class DeepLearningLayerSeg:
    ...
    def compute_segmentation_uncertainty(self, certainSlices):
        ...
        if self.octScan.get_scan_path() is not None:
            ...
            if len(d2) == 7:
                ...
                self.entropyValsPerScan = list(
                    self.entropyValsPerScan.reshape((scans.shape[2], 2, self.octScan.get_dim()[1])))
                self.probabilityValsPerScan = list(
                    self.probabilityValsPerScan.reshape((scans.shape[2], 2, self.octScan.get_dim()[1])))
            ...
            # ConvertToColors:
            if True:
                self.probabilityColors = np.empty(
                    (len(self.probabilityValsPerScan), len(self.probabilityValsPerScan[0][0]), 3, 2))
    ...
    def compute_shortest_path_in_A_scan_direction(self, i, j, sliceNumZ, smoothness, neighborhood):
        ...
        useMin = False
        if useMin:
            return self.find_shortest_path_in_A_scan_direction_using_local_min(probMaps, i, j, sliceNumZ, smoothness,
                                                                               neighborhood)
        else:
